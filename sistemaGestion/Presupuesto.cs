...
namespace sistemaGestion
{
    public partial class Presupuesto : Form
    {
        ...
        public Presupuesto(Form1 p, string id)
        {
            InitializeComponent();
            padre = p;
            idUsuario = id;
        ...
        private void textBox4_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (dbCon.IsConnect())
                {
                    ...
                    {
                        MessageBox.Show("Cliente no encontrado", "Presupuestos", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        textBox4.Text = "1";
        ...
        {
           float subtotal = 0; float total = 0;
            float descuento = 0; float recargo = 0;
            var cmd = new MySqlCommand();
            string query;
            subtotal = float.Parse(textBox8.Text);
            ...
            if (total != 0)
            {
                query = "INSERT INTO `presupuesto`(`presucodigo`, `presufecha`, `clicodigo`, `presusubtotal`, `presurecargo`, `presudescuento`, `presutotal`, `presuplazo`, `presucondicion`, `presuobs`, `vendedor`) VALUES (NULL,'"+fecha+"',"+textBox4.Text+",'"+subtotal.ToString().Replace(',','.')+ "','" + recargo.ToString().Replace(',', '.') + "','" + descuento.ToString().Replace(',', '.') + "','" + total.ToString().Replace(',', '.') + "','"+textBox9.Text+"','"+comboBox1.Text+" // "+comboBox3.Text+"','"+ textBox12.Text + "',"+idUsuario+")";
                cmd = new MySqlCommand(query, dbCon.Connection);
                cmd.ExecuteNonQuery();
                ...
                for (int i = 0; i < dataGridView1.Rows.Count; i++)
                {
                    ...
                    {
                        query = "INSERT INTO `detallepresu`(`presucodigo`, `procodigo`, `prodescripcion`, `procantidad`, `preciounitario`, `subtotal`) VALUES ("+codigo.ToString()+","+dataGridView1.Rows[i].Cells[5].Value.ToString()+ ",'" + dataGridView1.Rows[i].Cells[1].Value.ToString() + "','" + dataGridView1.Rows[i].Cells[2].Value.ToString().Replace(',','.') + "','" + dataGridView1.Rows[i].Cells[3].Value.ToString().Replace(',', '.') + "','" + dataGridView1.Rows[i].Cells[4].Value.ToString().Replace(',', '.') + "')";
                        cmd = new MySqlCommand(query, dbCon.Connection);
                        cmd.ExecuteNonQuery();
                    }
                }
